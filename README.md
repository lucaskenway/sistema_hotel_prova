...
> Infraestrutura: **Docker Compose (avaliação)** · Kubernetes (produção)
...
|---|---|---|
| `hotel_postgres` | `postgres:17` | Banco de dados PostgreSQL |
| `hotel_node` | build local (`Dockerfile`) | Servidor Node.js 24 (API REST) — acesso apenas via Nginx |
| `hotel_nginx` | build local (`docker/nginx/Dockerfile`) | Proxy reverso — único ponto de entrada externo (porta 80) |
| `hotel_redis` | `redis:7-alpine` | Cache |
...
**Pré-requisito:** Docker instalado (`docker --version`).
...
Aguarde até o log mostrar que o Nginx e o backend estão prontos. Para verificar:
...
Acesse no navegador após subir os containers:
...
**1. Criar um hotel e usuário administrador**
...
Adicione o header em todas as chamadas (exceto `/auth/login` e `/auth/register`):

```
Authorization: Bearer <token_aqui>
```

**No Swagger:** clique em **Authorize** (canto superior direito) → cole o token → confirme.
